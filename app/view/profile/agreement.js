...
export default class Agreement extends BaseComponent {
  ...
  _onScroll({ nativeEvent }) {
    ...
    if (offset > 215) {
      this.setState({
        header: { backgroundColor: Variable.blue }
      })
    } else if (offset < 50) {
      this.setState({
        header: null
  ...
  render() {
    return (
        <View style={{ flex: 1 }}>
          <View style={[styles.header, this.state.header]}>
          ...
          <ScrollView style={styles.wrapper}
            ...
            <ImageBackground source={require('../../res/image/xieyi_bg.png')}
                             style={styles.bgImg} />
            <View style={styles.content}>
              <Text style={styles.title}>晚安日记用户协议</Text>
              <Text style={styles.text}>
                感谢您下载晚安日记！当您开始使用本软件时，我
                们可能会对您的部分个人信息进行收集、使用、共
                享和保护。为了让您更好地了解我们对您信息的使
                用和保护。我们依法更新隐私政策，请您在使用晚
                安日记产品或服务前，认真阅读完整的《晚安日记
                隐私政策》。{'\n\n'}
